...
public interface CentralAuthenticationService {
    ...
    /**
     * Create a {@link org.jasig.cas.ticket.TicketGrantingTicket} by
     * authenticating credentials. The details of the security policy around
     * credential authentication and the definition of authentication success
     * are dependent on the implementation, but it SHOULD be safe to assume that
     * at least one credential MUST be authenticated for ticket creation to
     * succeed.
     * 
     * @param credentials One or more credentials that may be authenticated in
     *            order to create the ticket.
    ...
    /**
     * Grants a {@link org.jasig.cas.ticket.ServiceTicket} that may be used to
     * access the given service.
     * 
    ...
    /**
     * Grant a {@link org.jasig.cas.ticket.ServiceTicket} that may be used to
     * access the given service by authenticating the given credentials. The
     * details of the security policy around credential authentication and the
     * definition of authentication success are dependent on the implementation,
     * but it SHOULD be safe to assume that at least one credential MUST be
     * authenticated for ticket creation to succeed.
     * <p>
     * The principal that is resolved from the authenticated credentials MUST be
     * the same as that to which the given ticket-granting ticket was issued.
     * </p>
     * 
     * @param ticketGrantingTicketId Proof of prior authentication.
     * @param service The target service of the ServiceTicket.
     * @param credentials One or more credentials to authenticate prior to
     *            granting the service ticket.
     * @return Non-null service ticket identifier.
     * @throws AuthenticationException on errors authenticating the credentials
     * @throws TicketException if the ticket could not be created.
     */
     ...
     */
    Assertion validateServiceTicket(final String serviceTicketId, final Service service, final String sysName)
     ...
     * to other Services.
     * 
     * @param serviceTicketId The service ticket identifier that will delegate
     *            to a {@link org.jasig.cas.ticket.TicketGrantingTicket}.
     * @param credentials One or more credentials to authenticate prior to
     *            delegating the ticket.
     * @return Non-null ticket-granting ticket identifier that can grant
     *         {@link org.jasig.cas.ticket.ServiceTicket} that proxy
     *         authentication.
     * @throws AuthenticationException on errors authenticating the credentials
