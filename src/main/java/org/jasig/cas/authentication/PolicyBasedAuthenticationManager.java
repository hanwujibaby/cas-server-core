...
/**
 * Provides an authenticaiton manager that is inherently aware of multiple
 * credentials and supports pluggable security policy via the
 * {@link AuthenticationPolicy} component. The authentication process is as
 * follows:
 * <ul>
 ...
 * <ul>
 * <li>Check whether a resolver is configured for the handler that authenticated
 * the credential.</li>
 ...
 * </ul>
 * </ul> <li>After all credentials have been attempted check security policy
 * again. Note there is an implicit security policy that requires at least one
 * credential to be authenticated. Then the security policy given by
 * {@link #setAuthenticationPolicy(AuthenticationPolicy)} is applied. In all
 * cases {@link AuthenticationException} is raised if security policy is not
 * met.</li> </ul> It is an error condition to fail to resolve a principal.
 * 
...
public class PolicyBasedAuthenticationManager implements AuthenticationManager {
    ...
    /**
     * Default principal implementation that allows us to create
     * {@link Authentication}s (principal cannot be null).
     */
     ...
     * 
     * @param handlers Non-null list of authentication handlers containing at
     *            least one entry.
     */
     ...
     * used.
     * 
     * @param map Non-null map of authentication handler to principal resolver
     ...
     * 
     * @param policy Non-null authentication policy. The default policy is
     ...
     *         failure.
     * @throws AuthenticationException When one or more credentials failed
     *             authentication such that security policy was not satisfied.
     */
            ...
            for (final AuthenticationHandler handler : this.handlerResolverMap.keySet()) {
                if (handler.supports(credential)) {
                    ...
                    try {
                        ...
                        logger.debug("hander===>" + handler.getName() + "|credential:" + credential.getClass()
                                + "result==>" + result.getHandlerName());
                        ...
                        if (resolver == null) {
                            ...
                            logger.debug("No resolver configured for {}. Falling back to handler principal {}", handler
                                    .getName(), principal);
                        } else {
     ...
     * @return Authentication exception containing information about
    ...
    /**
     * Null prinicpal implementation that allows us to construct
     * {@link Authentication}s in the event that no principal is resolved during
